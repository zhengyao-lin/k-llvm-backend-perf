data Nat = D | N0 Nat  | N1 Nat  | N2 Nat  | N3 Nat  | N4 Nat  | N5 Nat  | N6 Nat  | N7 Nat  | N8 Nat  | N9 Nat 
...
prec (N9 x) = (N8 x)
fact x | x == D = (N1 D)
fact x | x /= D = (times x (fact (prec x)))
fib x | x == D = (N1 D)
fib x | x /= D && x == (N1 D) = (N1 D)
fib x | x /= D && x /= (N1 D) = (plus (fib (prec x)) (fib (prec (prec x))))
reduce D = D
reduce (N0 x) | (reduce x) == D = D
reduce (N0 x) | (reduce x) /= D = (N0 (reduce x))
