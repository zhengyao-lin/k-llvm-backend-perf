...
public class Fibfree {
  %gom{
    ...
    Xnat =
      ...
      | mult5(rec_x1_8:Xnat)
      | mult6(rec_x1_9:Xnat)
      | mult7(rec_x1_10:Xnat)
      | mult8(rec_x1_11:Xnat)
      | mult9(rec_x1_12:Xnat)
      | n0(rec_x1_13:Xnat)
      | n1(rec_x1_14:Xnat)
      | n2(rec_x1_15:Xnat)
      | n3(rec_x1_16:Xnat)
      ...
      | n8(rec_x1_21:Xnat)
      | n9(rec_x1_22:Xnat)
      | plus(rec_x1_23:Xnat, rec_x2_23:Xnat)
      | prec(rec_x1_24:Xnat)
      | reduce(rec_x1_25:Xnat)
      | times(rec_x1_26:Xnat, rec_x2_26:Xnat)
    module Term:rules() {
      plus (X, d()) -> X
      plus (d(), X) -> X
      ...
      prec (n9 (X)) -> n8 (X)
      fact (X) -> n1 (d()) if X == d()
      fact (X) -> times (X, fact (prec (X))) if X != d()
      fib (X) -> n1 (d()) if X == d()
      fib (X) -> n1 (d()) if X != d() && X == n1 (d())
      fib (X) -> plus (fib (prec (X)), fib (prec (prec (X)))) if X != d() && X != n1 (d())
      reduce (d()) -> d()
      reduce (n0 (X)) -> d() if reduce (X) == d()
      reduce (n0 (X)) -> n0 (reduce (X)) if reduce (X) != d()
