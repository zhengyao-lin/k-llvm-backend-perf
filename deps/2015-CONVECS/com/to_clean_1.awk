...
BEGIN {
	print "#! /bin/sh"
	# do not replace "->" by "=" because "->" is used in function profiles
	print "sed -e 's+-><-+==+g' |" 
	print "sed -e 's+->/<-+<>+g' |" 
...
END {
	...
	# expressions "f (x1, ..., xn)" to Clean expressions
	# "f x1 ... xn" (this code was derived from the Haskell 
	# translator by modifying step 5)
	...
	print "sed -e 's/^\\([^|]*\\))[ ]*= /\\1 = /' |"
	# step 5: replace ")" by "\n" before " | " (for conditional equations)
	print "sed -e 's/)[ ]* | / \\n| /' |"
	print "cat"
}
